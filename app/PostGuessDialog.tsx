...
export default function PostGuessDialog({
	...
	const gainedStars = useMemo(() => calculateStars(guess, answer), [guess, answer]);
			...
			<div className="fixed inset-0 flex items-center justify-center">
				<Dialog.Panel className="w-full max-w-2xl rounded-md bg-ctp-mantle p-2 space-y-2">
					...
					<p className="text-center">
						You gained <strong>{calculateStars(guess, answer)}{" "}
							{pluralize("star", gainedStars)}</strong>
					</p>

					<Suspense fallback={<div className="h-40 w-full bg-ctp-crust animate-pulse" />}>
